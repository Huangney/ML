...
def chat():
    ...
    word_dict_path = os.path.join(path,word_dict_name)

    try:
        word_dict_file = open(word_dict_path,'r',encoding='utf-8')
        word_index = json.load(word_dict_file)
    ...
    word_size = len(word_index) + 1 # 因为0是预留位置 所以+1
    ...
    items = [word_size,encoder,decoder,limit,tok,embedding_dim,word_index]
